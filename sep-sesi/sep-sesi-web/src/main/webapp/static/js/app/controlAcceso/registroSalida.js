$(function() {
     init();
     $('#keyPersona').focus();
 });

function init(){     
    imgPredeterminada();
    obtenerDatosRol();     
}    

function obtenerDatosRol(){
    var vUrl = baseURI + '/mvc/seguridad/menu/obtenerUsuario';
    conectaPost(vUrl,"",resUsuario);
}

 function resUsuario(data){     
    var idUsuario = data.idUsuario;
    var idInmueble = data.idInmueble;
    var idAcceso = data.idAcceso;
    var inmueble = data.descInmueble;
    var acceso = data.descAcceso;    
    
    $("#idUsuarioSal").val( idUsuario );
    $("#idAccesoSal").val( idAcceso );  
    $("#accesoSal").val( acceso );
    $("#idInmuebleSal").val( idInmueble );
    $("#descInmuebleSal").val( inmueble );
    
 }
 
function bin2string(array) {
    var result = "";
    for (var i = 0; i < array.length; ++i) {
        result += (String.fromCharCode(array[i]));
    }
    return result;
}

$("#keyPersona").change(function(e) {
    //e.preventDefault();    
    var a = $(this).val();    
    if( a != '' ){          
        var separador = '';     
        for (let character in a) {  
            if( a[character] === "'"  ){
               separador = "'";
               break;
            }
            if( a[character] === "-"  ){
               separador = "-";
               break;
            }
        }       
        
        var qr = a.split( separador ); 
        if( qr[0].length == 7 && qr[1] !== 'undefined' && qr[1].length > 0 ){            
            registrarSalidaPersona(qr[0],qr[1]); 
        } else {
            alert('El valor del c\u00F3digo QR ingresado es incorrecto');
            $(this).val('');
            return false;            
        }   
    }       
});

function registrarSalidaPersona(identificador, tipoRegistro){   
    $('#loader').modal('toggle');
    
    var url = "";
    var idUsuario = $('#idUsuarioSal').val();
    var idAcceso = $('#idAccesoSal').val();
    var idInmueble = $('#idInmuebleSal').val();
    var tipoPersona = tipoRegistro;   
    
    var data = {};    
    if( tipoPersona == 3 ){
        url = baseURI + '/mvc/controlAcceso/registrarSalidaVisitante';
        data["idEtiqueta"] = identificador;
    } else {
        url = baseURI + '/mvc/controlAcceso/registrarSalidaPersona';
        data["idPersona"] = identificador;
    }
    data["idUsuario"] = idUsuario;
    data["idAcceso"] = idAcceso;
    data["idInmueble"] = idInmueble;
    
    $('#keyPersona').val('');   
    $('#keyPersona').prop( "disabled", true );
    $.ajax({
        url: url,
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify(data),
        dataType: "json",
        cache: false,      
        complete : function(xhr, statusText){
            var status = xhr.status;
            if( status != 200 ){
                imgPredeterminada();
                if( status == 423 ){                
                    $('#nombrePersonaSal').text('La persona no se encuentra activa. Pasar al m\u00F3dulo de registro de visitante'); 
                } else {
                    $('#nombrePersonaSal').text("La persona no se encuentra registrado. Pasar al m\u00F3dulo de registro de visitante");
                }            
                setTimeout(function() {                    
                    $('#nombrePersonaSal').text('');
                    imgPredeterminada();
                }, 5000);
                $('#keyPersona').prop( "disabled", false );
                $('#keyPersona').focus();
            }
        },
        success : function(data) {     
            var nombre = data.nombre;
            var primerApellido = data.apellido1 == null ? '':data.apellido1;
            var segundoApellido = data.pellido2 == null ? '':data.apellido2;              
            var fotografia = data.fotografia;            
            if(fotografia == null || fotografia == ''){                
                fotografia = imagenBase();                 
            }     
            document.getElementById("ItemPreviewSal").src = "data:image/jpg;base64," + fotografia;
            document.getElementById("ItemPreviewSal").width = 300;
            document.getElementById("ItemPreviewSal").height = 300;  
            $('#nombrePersonaSal').text(nombre + ' ' + primerApellido + ' ' + segundoApellido);
            $('#loader').modal('toggle');                 
            eventoResultado();                       
        },
        error : function(jqXHR, textStatus, errorThrown) {            
            $('#loader').modal('toggle');
            $('#keyPersona').prop( "disabled", false );
            $('#keyPersona').focus();
        }
     });
}

function imagenBase(){    
    var imagen = "";
    return imagen;
}

function eventoResultado(){
    $("#registroCorrectoSal").show();
    setTimeout(function() {
        $('#registroCorrectoSal').fadeOut('fast');
    }, 5000);
    
    setTimeout(function() {        
        $('#nombrePersonaSal').text('');
        imgPredeterminada();
    }, 5000);
    $('#keyPersona').prop( "disabled", false );
    $('#keyPersona').focus();
}

function imgPredeterminada(){
    var fotografia = imagenBase();  
    document.getElementById("ItemPreviewSal").src = "data:image/jpg;base64," + fotografia;
    document.getElementById("ItemPreviewSal").width = 300;
    document.getElementById("ItemPreviewSal").height = 300;
}

$('#divCodigoQR').click(function(e){
    e.preventDefault();
    $('#keyPersona').focus();    
});

