...
{
    var idUR = $("#ur").val();
    location.href = baseURI + "/mvc/persona/consultaEmpleado?idUR=" + idUR;
}

function consultaEmpleados()
{
    location.href = baseURI + "/mvc/persona/consultaEmpleados";
}
...
function resCuadrantes(data) {
    console.log('resCuadrantes ' + data);
...
function insertaEmpleado() {
    ...
    /*var vcfData = new FormData($('#formCarga')[0]);
     ...
     $('#loader').modal('toggle');
     $('#myModal1').modal('show');
     $("#myModal1 .modal-body").text('Ocurri&oacute; un error al registrar los datos');
     console.log('jqXHR.status:: ' + jqXHR.status);
     console.log('jqXHR.text:: ' + jqXHR.text);
     console.log('textStatus:: ' + textStatus);
     }
...
/*function limpiaEtiquetasError(){
 ...
 $("#errorEstatus").hide(); 
 $("#curpDiv").parent("div.form-group").removeClass("has-error");
...
/*function validaSoloLetrasPuesto(data) {
 var re = /^(?=.{1,40}$)[a-zA-Z\u00F1\u00D1\u00DC]+(?:[-_<>' ][a-zA-Z\u00F1\u00D1\u00DC]+)*$/;
