window.addEventListener ?
        window.addEventListener("load", obtieneDatos, false) :
        window.attachEvent && window.attachEvent("onload", obtieneDatos);

function obtieneDatos() {
    
    var x = document.getElementById("datosPersona");
    x.style.display = "none";
    verificarDatos();
        
    var columnas = [
            {
                field: 'nombre',
                title: 'Marca',
                align: 'center'
            },
            {
                field: 'puesto',
                title: 'Número de serie',
                align: 'center'
            }]
       /* $('#resultadosResumenTable').bootstrapTable({
            columns: columnas
        });*/
        //$('#resultadosResumenTable').bootstrapTable('showLoading');
        $('#resultadosResumenTable').bootstrapTable('hideLoading');

    var pathname = window.location.pathname;

    var sURLVariables = pathname.split('vE1.jsp/');
    //var datos = cadena.split("/ValidacionCredencial/"); 
    var cadena = sURLVariables[sURLVariables.length-1];
    //console.log('cadena ' + cadena);
    var param = {};
        param["cadena"] = cadena;
    $('#loader2').modal('toggle');
    $.ajax({
        type: "GET",
        cache: false,
        url: baseURI + "/mvc/consultaQR/datosPersonaEquipos",
        //url: baseURI + "/mvc/consultaQR/generarQREquipos",
        contentType: "application/json; charset=utf-8",
        data: param,
        cache: false,
        success: function (response) {
            
            if (response.idPersona !== '') {
                var fotografia = response.fotografia;
            if(fotografia == null || fotografia == ''){                
                fotografia = imagenBase();                 
            }     
            document.getElementById("ItemPreview").src = "data:image/jpg;base64," + fotografia;
            document.getElementById("ItemPreview").width = 300;
            document.getElementById("ItemPreview").height = 300;
            var nombre = response.nombre;
            var primerApellido = response.apellido1;
            var segundoApellido = response.apellido2;
            if (segundoApellido == null){
                segundoApellido = '';
            }
            $('#ur').val(response.unidadResponsable);
            $('#nombre').val(nombre + " " + primerApellido + " " + segundoApellido);
            $('#puesto').val(response.puesto);
            $('#estatus').val(response.estatusDes);
            gridEquipos();
            //obtInmueble();
            /*var usuarioLogin = $('#usuario').val();
            console.log('usuarioLogin ' + usuarioLogin);
            if (usuarioLogin === null || usuarioLogin.length === 0) {
                document.getElementById("usuarioQr").style.display = "block";
                document.getElementById("passwordID").style.display = "block";
            }
            else{
                document.getElementById("usuarioQr").style.display = "none";
                document.getElementById("passwordID").style.display = "none";
            }*/
            $('#loader2').modal('toggle');
            }

        },
        error: function (jqXHR, textStatus, errorThrown) {
            $('#loader2').modal('toggle');
            console.log('jqXHR.status:: ' + jqXHR.status);
            console.log('jqXHR.text:: ' + jqXHR.text);
            console.log('textStatus:: ' + textStatus);
        }
        });
        
        

}

function gridEquipos(){    
                //object.configModule.standby.show();
                var columnas = [
                {
                field: 'marca',
                title: 'Marca',
                align: 'center'
            },
            {
                field: 'numSerie',
                title: 'N\u00FAmero de serie',
                align: 'center'
            },{
                field: 'partida',
                title: 'Descripci\u00F3n del equipo',
                align: 'center'
            }]
                $('#resultadosResumenTable').bootstrapTable({
                    columns : columnas
                });
                $('#resultadosResumenTable').bootstrapTable('hideLoading');
               var pathname = window.location.pathname;

    var sURLVariables = pathname.split('vE1.jsp/');
    //var datos = cadena.split("/ValidacionCredencial/"); 
    var cadena = sURLVariables[sURLVariables.length-1];
    var param = {};
        param["cadena"] = cadena;
                //$('#loader').modal('toggle');
        	$.ajax({
                    url: baseURI +"/mvc/consultaQR/datosEquipo",
                    type: 'GET',
                    contentType: "application/json; charset=utf-8",
                    data: param,
                    //dataType: "json",
                    cache: false,
		}).done(function(resumenInstancias) {
                    //$('#idEquipo').val(resumenInstancias[0].id_equipo);
                    
                    //$('#resultadosResumenTable').bootstrapTable('hideLoading');
                    $('#resultadosResumenTable').bootstrapTable('destroy');
                    $('#resultadosResumenTable').bootstrapTable({
                        /*formatNoMatches: function () {
                            console.log('noMatches');
                            $('#btnGuardar').prop( "disabled", true );
                            return "No se encontraron registros";
                        },*/
                        formatNoMatches: noMatches,
                        formatLoadingMessage: function() {
                            return '';
                        },
                        pagination : false,
                        paginationHAlign : 'left',
                        paginationDetailHAlign : 'right',
                        classes: 'table table-bordered table-hover',
                        striped : true,
                        pageList : [],
                        columns : columnas,
                        data : resumenInstancias
                    });
                    $('#idEquipo').val(resumenInstancias[0].id_equipo);
                    $('#loader').modal('toggle');
		}).fail(function() {
                    $('#loader').modal('toggle');
		});
            };  

function noMatches(){
    $('#btnGuardar').prop( "disabled", true );
    $('#resultadosResumenTable').bootstrapTable('hideLoading');
    return 'No se encontraron registros';
    
}

function imagenBase(){    
    var imagen = "";
    return imagen;
}

function obtInmueble()
{
    $.ajax({
        type: "GET",
        cache: false,
        url: baseURI + "/mvc/catalogos/obtenerInmueble",
        contentType: "application/json; charset=utf-8",
        dataType: 'json',
        success: function (response) {

            if (response.length > 0) {

                var inmuebles;
                inmuebles = response;
                populateSelectInmueble(inmuebles);
            } else {

            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            $('#mensajesError').html('<div class="alert alert-warning">No se encontraron registros</div>').show();
            console.log('jqXHR.status:: ' + jqXHR.status);
            console.log('jqXHR.text:: ' + jqXHR.text);
            console.log('textStatus:: ' + textStatus);
        }
    });
}

function populateSelectInmueble(lstInmueble) {

    document.getElementById("inmuebleID").options.length = 0;
    for (var i = 0; i <= lstInmueble.length - 1; i++) {
        if (i == 0 && lstInmueble.length >= 1) {
            $('#inmuebleID').append('<option value="">' + 'SELECCIONA' + '</option>');
        }
        $('#inmuebleID').append('<option value="' + lstInmueble[i].idInmueble + '">' + lstInmueble[i].denominacion + '</option>');
    }

}

$("#keyPersona").change(function(e) {
    //e.preventDefault();       
    var cadena = $("#keyPersona").val();
    var url = "";
    
    if( cadena !== 'undefined' && cadena.trim().length >= 7 ){           
        if(cadena.substring(0,4) === 'http'){            
            var datos = cadena.split("/vE1.jsp/"); 
            cadena = datos[datos.length-1];
            url = baseURI + '/mvc/controlAcceso/registrarEntrada';
            
            //registrarEntradaPersona(url,cadena,4);
        }  else { 
            var dato =  cadena.substring(0,7);      
            var esNumerico = /^[0-9]+$/;
            if( dato.match( esNumerico ) ) {
                var separador = '';     
                for (let character in cadena) {  
                    if( cadena[character] === "'"  ){
                       separador = "'";
                       break;
                    }
                    if( cadena[character] === "-"  ){
                       separador = "-";
                       break;
                    }
                }              
                var qr = cadena.split( separador ); 
                if(  qr.length >= 2 && qr[0].length == 7 ){
                    if( qr[1] == 3 ){
                        url = baseURI + '/mvc/controlAcceso/registrarEntradaVisitante';
                    }else{
                        url = baseURI + '/mvc/controlAcceso/registrarEntradaPersona';
                    }  
                    //registrarEntradaPersona(url,qr[0],qr[1]);   
                } else {
                    alert('El valor del c\u00F3digo QR ingresado es incorrecto');
                    $(this).val('');
                    return false;             
                }
            } else {                
                url = baseURI + '/mvc/consultaQR/recuperarDatosProveedor';
                recuperaDatosProveedor(url,cadena,4);
            }
        }           
    } else {
        alert('El valor del c\u00F3digo QR ingresado es incorrecto');
        $(this).val('');
        return false;  
    }    
});

function recuperaDatosProveedor(url, identificador, tipoPersona){   
    $('#loader').modal('toggle');
        
    var data = {};   
    if( tipoPersona == 1  || tipoPersona == 2 ){ //Empleado o proveedor         
        data["idPersona"] = identificador;
    } else if( tipoPersona == 3 ){ //Visitante
        data["idEtiqueta"] = identificador;
    } else { //Cadena encriptada
        data["cadenaQR"] = identificador;
    }     
   // console.log('Solicitando acceso: ' + idUsuario + "-"+ idAcceso+"-"+idInmueble);
    //$('#keyPersona').val('');   
    $('#keyPersona').prop( "disabled", true );
    $.ajax({
        url: url,
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify(data),
        dataType: "json",
        cache: false,      
        complete : function(xhr, statusText){   
        },
        success : function(data) {        
            $('#curpProveedor').val(data.curp);
            var inmueble = data.idInmueble;
            obtInmuebleMod(inmueble);
            //$('#inmuebleID').val(inmueble);
            $('#loader').modal('toggle');                                      
        },
        error : function(jqXHR, textStatus, errorThrown) { 
            $('#loader').modal('toggle');
            $('#keyPersona').prop( "disabled", false );
            $('#keyPersona').focus();
        }
     });
}


function obtInmuebleMod(idInmueble)
{
    $.ajax({
        type: "GET",
        cache: false,
        url: baseURI + "/mvc/catalogos/obtenerInmueble",
        contentType: "application/json; charset=utf-8",
        dataType: 'json',
        success: function (response) {

            //drawTableGeneric(response);
            if (response.length > 0) {

                var inmuebles;
                inmuebles = response;
                populateSelectInmuebleMod(inmuebles, idInmueble);
            } else {

            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            $('#mensajesError').html('<div class="alert alert-warning">No se encontraron registros</div>').show();
            console.log('jqXHR.status:: ' + jqXHR.status);
            console.log('jqXHR.text:: ' + jqXHR.text);
            console.log('textStatus:: ' + textStatus);
        }
    });
}

function populateSelectInmuebleMod(lstInmueble, idInmueble) {

    document.getElementById("inmuebleID").options.length = 0;
    for (var i = 0; i <= lstInmueble.length - 1; i++) {
        if (i == 0 && lstInmueble.length >= 1) {
            $('#inmuebleID').append('<option>' + 'SELECCIONA' + '</option>');
        }
        $('#inmuebleID').append('<option value="' + lstInmueble[i].idInmueble + '">' + lstInmueble[i].denominacion + '</option>');
    }
    $('#inmuebleID').val(idInmueble);

}

function guardaEntradaSalidaEquipo() {
    //e.preventDefault();       
    var cadena = $(this).val(); 
    var url = "";
    
    if( cadena !== 'undefined' && cadena.trim().length >= 7 ){           
        if(cadena.substring(0,4) === 'http'){            
            var datos = cadena.split("/vC.jsp/"); 
            cadena = datos[datos.length-1];
            url = baseURI + '/mvc/controlAcceso/registrarEntrada';
            registrarEntradaPersona(url,cadena,4);
        }  else { 
            var dato =  cadena.substring(0,7);      
            var esNumerico = /^[0-9]+$/;
            if( dato.match( esNumerico ) ) {
                var separador = '';     
                for (let character in cadena) {  
                    if( cadena[character] === "'"  ){
                       separador = "'";
                       break;
                    }
                    if( cadena[character] === "-"  ){
                       separador = "-";
                       break;
                    }
                }              
                var qr = cadena.split( separador ); 
                if(  qr.length >= 2 && qr[0].length == 7 ){
                    if( qr[1] == 3 ){
                        url = baseURI + '/mvc/controlAcceso/registrarEntradaVisitante';
                    }else{
                        url = baseURI + '/mvc/controlAcceso/registrarEntradaPersona';
                    }  
                    registrarEntradaPersona(url,qr[0],qr[1]);   
                } else {
                    alert('El valor del c\u00F3digo QR ingresado es incorrecto');
                    $(this).val('');
                    return false;             
                }
            } else {                
                
                url = baseURI + '/mvc/consultaQR/registraOperacionEquipos';
                registrarEntradaEquipos(url,cadena,4);
            }
        }           
    } else {
        alert('El valor del c\u00F3digo QR ingresado es incorrecto');
        $(this).val('');
        return false;  
    }    
};


function registrarEntradaEquipos(){  
    if (validaDatos()){
    $('#loader').modal('toggle');
    
    //var inmueble = document.getElementById("inmuebleID");
    //var idInmueble = inmueble.value;
    //var idInmueble = $('#idInmueble').val();
    var tipoOperacion = $('#operacionID').val();
    var idEquipo = $('#idEquipo').val();
    var usuario = $('#usuarioID').val();
    var pass = $('#contrasegnaUsuar').val();
    var bandera = $('#bandera').val();
    
    var data = {};   
    data["tipoOperacion"] = tipoOperacion;
    data["idEquipo"] = idEquipo;
    //data["idInmueble"] = idInmueble;   
    data["usuario"] = usuario;
    data["password"] = pass;
    data["bandera"] = bandera;
   // console.log('Solicitando acceso: ' + idUsuario + "-"+ idAcceso+"-"+idInmueble);
    $('#btnGuardar').prop( "disabled", true );
    $.ajax({
        url: baseURI + '/mvc/consultaQR/registraOperacionEquipos',
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify(data),
        dataType: "json",
        cache: false,      
        success : function(data) {                

                var respuesta = data.msg;
                                if (respuesta === '1') {
                                    alert("No cuenta con los permisos para registrar entrada/salida");                                   
                                    $('#btnGuardar').prop( "disabled", false );
                                }
                                else if (respuesta === '2') {
                                    alert("Usuario o contrase\u00F1a incorrectos");
                                    $('#btnGuardar').prop( "disabled", false );
                                }
                                else if (respuesta === '3') {
                                    alert("Se tiene que registrar la entrada del equipo");
                                    $('#btnGuardar').prop( "disabled", false );
                                }
                                else if (respuesta === '4') {
                                    alert("Se tiene que registrar la salida del equipo");
                                    $('#btnGuardar').prop( "disabled", false );
                                }
                                else {
                                    alert(respuesta);
                                    
                                    setTimeout(function () {
                                        location.href = baseURI + '/sep-sesi-web/login.jsp';
                                    }, 3000);
                                }   
                                
        },
        error : function(jqXHR, textStatus, errorThrown) { 
            $('#loader').modal('toggle');
            $('#keyPersona').prop( "disabled", false );
            $('#keyPersona').focus();
        }
     });
    }
}

function verificarDatos(){
    var usuario = $('#usuarioID').val();
    var pass = $('#contrasegnaUsuar').val();
    var param = {};
        param["usuario"] = usuario;
        param["pass"] = pass;
    if ((usuario === null || usuario.length === 0 ) || (pass === null || pass.length === 0)) {  
            var x = document.getElementById("datosPersona");
            x.style.display = "none";
        }
        else{
            $.ajax({
        type: "GET",
        cache: false,
        url: baseURI + "/mvc/consultaQR/verificaCredenciales",
        contentType: "application/json; charset=utf-8",
        data: param,
        dataType: "json",
        cache: false,
        success: function (response) {
            
            var respuesta = response.msg;
            if(respuesta === '1'){
                var x = document.getElementById("datosPersona");
                x.style.display = "block";
            }
            else{
                var x = document.getElementById("datosPersona");
                x.style.display = "none";
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            //$('#loader2').modal('toggle');
            console.log('jqXHR.status:: ' + jqXHR.status);
            console.log('jqXHR.text:: ' + jqXHR.text);
            console.log('textStatus:: ' + textStatus);
        }
        });
           
        }
    
}

function eventoResultado(){
    $("#registroCorrecto").show();
    setTimeout(function() {
        $('#registroCorrecto').fadeOut('fast');
    }, 5000);
    
   /* setTimeout(function() {        
        $('#nombrePersona').text('');
        imgPredeterminada();
    }, 5000);
    $('#keyPersona').prop( "disabled", false );
    $('#keyPersona').focus();*/
}

function validaDatos() {
    
    var datosValidos = true;
    var tipoOperacion = $('#operacionID').val();
    var usuario = $('#usuarioID').val();
    var pass = $('#contrasegnaUsuar').val(); 
    //var inmueble = document.getElementById("inmuebleID");
    //var idInmueble = inmueble.value;

    if ($('#usuarioQr').is(':visible')) {
        if (usuario === null || usuario.length === 0) {
            $("#usuarioQr").addClass("has-error");
            $("#errorUsuario").text('');
            $("#errorUsuario").show();
            $("#errorUsuario").append("Campo obligatorio");
            datosValidos = false;
        }
    
        if (pass === null || pass.length === 0) {
            $("#passwordID").addClass("has-error");
            $("#errorPassword").text('');
            $("#errorPassword").show();
            $("#errorPassword").append("Campo obligatorio");
            datosValidos = false;
        }
        $('#bandera').val("0");
    }else{
       $('#bandera').val("1"); 
    }
    
    /*if (idInmueble === "") {
        $("#inmuebleDiv").addClass("has-error");
        $("#errorInmueble").text('');
        $("#errorInmueble").show();
        $("#errorInmueble").append("Campo obligatorio");
        datosValidos = false;
    }*/
    
    if (tipoOperacion === "SELECCIONA") {
        $("#operacionDiv").addClass("has-error");
        $("#errorOperacion").text('');
        $("#errorOperacion").show();
        $("#errorOperacion").append("Campo obligatorio");
        datosValidos = false;
    }

    
    return datosValidos;
}

