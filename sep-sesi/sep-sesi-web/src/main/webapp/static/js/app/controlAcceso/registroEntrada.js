 $(function() {
    init();    
    $('#keyPersona').focus();
    $("#divLectorQR").animate({ scrollTop: $('#divLectorQR')[0].scrollHeight}, 1000);
 });

 function init(){     
    imgPredeterminada();
    obtenerDatosRol();     
 }  

function obtenerDatosRol(){
    var vUrl = baseURI + '/mvc/seguridad/menu/obtenerUsuario';
    conectaPost(vUrl,"",resUsuario);
}

 function resUsuario(data){     
    var idUsuario = data.idUsuario;
    var idInmueble = data.idInmueble;
    var idAcceso = data.idAcceso;
    var inmueble = data.descInmueble;
    var acceso = data.descAcceso;    
    
    $("#idUsuario").val( idUsuario );
    $("#idAcceso").val( idAcceso );
    $("#acceso").val( acceso );
    $("#idInmueble").val( idInmueble );
    $("#descInmueble").val( inmueble );
 }
 
function bin2string(array) {
    var result = "";
    for (var i = 0; i < array.length; ++i) {
        result += (String.fromCharCode(array[i]));
    }
    return result;
}

$("#keyPersona").change(function(e) {
    //e.preventDefault();       
    var cadena = $(this).val(); 
    var url = "";
    
    if( cadena !== 'undefined' && cadena.trim().length >= 7 ){           
        if(cadena.substring(0,4) === 'http'){         
            var datos = cadena.split("/vC.jsp/"); 
            cadena = datos[datos.length-1];
            url = baseURI + '/mvc/controlAcceso/registrarEntrada';;
            registrarEntradaPersona(url,cadena,4);
        }  else { 
            console.log('entramos al else');
            var dato =  cadena.substring(0,7); 
            console.log('dato es ' + dato);
            var esNumerico = /^[0-9]+$/;
            if( dato.match( esNumerico ) ) {
                var separador = '';     
                for (let character in cadena) {  
                    if( cadena[character] === "'"  ){
                       separador = "'";
                       break;
                    }
                    if( cadena[character] === "-"  ){
                       separador = "-";
                       break;
                    }
                }              
                var qr = cadena.split( separador ); 
                console.log('qr es ' + qr);
                if(  qr.length >= 2 && qr[0].length == 7 ){
                    if( qr[1] == 3 ){
                        url = baseURI + '/mvc/controlAcceso/registrarEntradaVisitante';
                    }else{
                        url = baseURI + '/mvc/controlAcceso/registrarEntradaPersona';
                    }  
                    registrarEntradaPersona(url,qr[0],qr[1]);   
                } else {
                    alert('El valor del c\u00F3digo QR ingresado es incorrecto');
                    $(this).val('');
                    return false;             
                }
            } else {                
                console.log('es un else');
                url = baseURI + '/mvc/controlAcceso/registrarEntrada';;
                registrarEntradaPersona(url,cadena,4);
            }
        }           
    } else {
        alert('El valor del c\u00F3digo QR ingresado es incorrecto');
        $(this).val('');
        return false;  
    }    
});


function registrarEntradaPersona(url, identificador, tipoPersona){   
    $('#loader').modal('toggle');
    
    var idUsuario = $('#idUsuario').val();
    var idAcceso = $('#idAcceso').val();
    var idInmueble = $('#idInmueble').val();
    
    var data = {};   
    if( tipoPersona == 1  || tipoPersona == 2 ){ //Empleado o proveedor         
        data["idPersona"] = identificador;
    } else if( tipoPersona == 3 ){ //Visitante
        data["idEtiqueta"] = identificador;
    } else { //Cadena encriptada
        data["cadenaQR"] = identificador;
    }    
    data["idUsuario"] = idUsuario;
    data["idAcceso"] = idAcceso;
    data["idInmueble"] = idInmueble;    
   // console.log('Solicitando acceso: ' + idUsuario + "-"+ idAcceso+"-"+idInmueble);
    $('#keyPersona').val('');   
    $('#keyPersona').prop( "disabled", true );
    $.ajax({
        url: url,
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify(data),
        dataType: "json",
        cache: false,      
        complete : function(xhr, statusText){   
            var status = xhr.status;
            if( status != 200 ){
                imgPredeterminada();
                if( status == 423 ){                                 
                    registroInactivo();
                } else if (status == 408){
                    sesionExpiro();
                } 
                else {                    
                    registroNoEncontrado();
                }            
                setTimeout(function() {    
                    $('#nombrePersona').text('');
                    imgPredeterminada();
                }, 5000);
                $('#keyPersona').prop( "disabled", false );
                $('#keyPersona').focus();
            }
        },
        success : function(data) {                
            var nombre = data.nombre;
            var primerApellido = data.apellido1 == null ? '':data.apellido1;
            var segundoApellido = data.pellido2 == null ? '':data.apellido2;              
            var fotografia = data.fotografia;
            if(fotografia == null || fotografia == ''){                
                fotografia = imagenBase();                 
            }     
            document.getElementById("ItemPreview").src = "data:image/jpg;base64," + fotografia;
            document.getElementById("ItemPreview").width = 300;
            document.getElementById("ItemPreview").height = 300;  
            $('#nombrePersona').text(nombre + ' ' + primerApellido + ' ' + segundoApellido);
            $('#loader').modal('toggle');               
            eventoResultado();                       
        },
        error : function(jqXHR, textStatus, errorThrown) { 
            $('#loader').modal('toggle');
            $('#keyPersona').prop( "disabled", false );
            $('#keyPersona').focus();
        }
     });
}

function imagenBase(){    
    var imagen = "";
    return imagen;
}

function eventoResultado(){
    $("#registroCorrecto").show();
    setTimeout(function() {
        $('#registroCorrecto').fadeOut('fast');
    }, 5000);
    
    setTimeout(function() {        
        $('#nombrePersona').text('');
        imgPredeterminada();
    }, 5000);
    $('#keyPersona').prop( "disabled", false );
    $('#keyPersona').focus();
}

function imgPredeterminada(){    
    var fotografia = imagenBase();  
    document.getElementById("ItemPreview").src = "data:image/jpg;base64," + fotografia;
    document.getElementById("ItemPreview").width = 300;
    document.getElementById("ItemPreview").height = 300;
}

$('#divCodigoQR').click(function(e){
    e.preventDefault();
    $('#keyPersona').focus();    
});

function registroInactivo(){
    $("#registroInactivo").show();
    setTimeout(function() {
        $('#registroInactivo').fadeOut('fast');
    }, 5000);   
   
}

function sesionExpiro(){
    $("#sesionExpirada").show();
    var sPaginaURL = location.origin; 
    ////var sPaginaURL = location.hostname;
    //var sPaginaURL = location.host;
    //var sPaginaURL = parent.window.location.href;
    
        console.log('baseURI' + sPaginaURL);
    setTimeout(function() {
        $('#sesionExpirada').fadeOut('fast');
        
        //parent.window.location.href =  baseURI + 'logout';
        window.location.href = sPaginaURL + '/sep-sesi-web/login.jsp';
    }, 7000);  
    
   
}

function registroNoEncontrado(){
    $("#noEncontrado").show();
    setTimeout(function() {
        $('#noEncontrado').fadeOut('fast');
    }, 5000);    
}

function obtenerValorParametro(sParametroNombre) {
    var sPaginaURL = window.location.search.substring(1);
    var sURLVariables = sPaginaURL.split('?');
    for (var i = 0; i < sURLVariables.length; i++) {
        var sParametro = sURLVariables[i].split('=');
        if (sParametro[0] == sParametroNombre) {
            return sParametro[1];
        }
    }
    return null;
}
